...
void initCameraPOS(long steps) { currentCameraPOS = steps; }
void initLaserPOS(long steps) { currentLaserPOS = steps; }
void initServoAngle(int angle) { currentServoAngle = constrain(angle, 0, 180); }
...
void updateServoAngle(int angle) {
  long temp_angle = constrain(currentServoAngle + angle, 0, 180);
...
void addServoAngle() {
  long temp_angle = constrain(currentServoAngle + 1, 0, 180);
  currentServoAngle = temp_angle;
}
